...
const updateBlog = async function (req, res) {
  try {
    ...
    if (!validator.isValidObjectId(blogId)) {
      res
        .status(400)
        .send({ status: false, message: `${blogId} is not a valid blog id` });
      return;
    }
    ...
    // Extract params
    const { title, body, tags, category, subcategory, isPublished } =
      requestBody;

    const updatedBlogData = {};

    if (validator.isValid(title)) {
      if (!Object.prototype.hasOwnProperty.call(updatedBlogData, "$set"))
        updatedBlogData["$set"] = {};
    ...
    if (tags) {
      if (!Object.prototype.hasOwnProperty.call(updatedBlogData, "$addToSet"))
    ...
    if (subcategory) {
      if (!Object.prototype.hasOwnProperty.call(updatedBlogData, "$addToSet"))
        updatedBlogData["$addToSet"] = {};
      if (validator.isArray(subcategory)) {
        updatedBlogData["$addToSet"]["subcategory"] = {
          $each: [...subcategory],
        };
      }
      if (validator.isValidString(subcategory)) {
...
const deleteBlogByParams = async function (req, res) {
  try {
    ...
    res
      .status(200)
      .send({ status: true, message: "Blog(s) deleted successfully" });
  } catch (error) {
    res.status(500).send({ status: false, message: error.message });
  }
};
